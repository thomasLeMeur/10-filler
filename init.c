...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/02/04 19:08:02 by tle-meur          #+#    #+#             */
/*   Updated: 2016/02/11 16:39:18 by tle-meur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	analyse_player(t_filler *f, char *s)
{
	int	id;

	id = s[10] - '0';
	f->mychars[0] = (id == 1) ? 'o' : 'x';
	f->mychars[1] = (id == 1) ? 'O' : 'X';
	f->itschars[0] = (id == 1) ? 'x' : 'o';
	f->itschars[1] = (id == 1) ? 'X' : 'O';
}
...
int			my_first_init(t_filler *f, char **line)
{
	...
	while (++f->y < f->grid.hig && (f->x = -1))
		while (++f->x < f->grid.wid)
			if (f->grid.g[f->y][f->x] == f->mychars[0]
					|| f->grid.g[f->y][f->x] == f->mychars[1])
			{
